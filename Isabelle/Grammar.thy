...
by - (rule Min_predicate, auto simp add: notr_nonempty notr_in_rules Set.image_subset_iff[symmetric])
...
proof -
  ...
  then show ?thesis using nov_nog'[OF G, of v n t] existence_from_lookup[OF A V] assms n_def
    by auto
qed
